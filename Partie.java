...
public class Partie implements Runnable, Serializable {
    ...
    public boolean canPlayTour (int posX, int posY, int dir, int pas) {
        ...
        if (dir == 1) { // UPMOV
            if (posX - pas < 0) return false;
            for (int i = posX-1; i >= posX - 1 - pas; i--) {
                if (grille[i][posY] == '|') return false;
            }
            return true;
        }
        else if (dir == 2) { // RIMOV
        if (posY + pas >= largeur) return false;
            for (int i = posY+1; i < posY + 1 + pas; i++) {
                if (grille[posX][i] == '|') return false;
            }
            return true;
        }
        else if (dir == 3) { // DOMOV
            if (posX + pas >= hauteur) return false;
                for (int i = posX+1; i < posX + 1 + pas; i++) {
                    if (grille[i][posY] == '|') return false;
                }
            return true;
    ...
    public void joueTour (Joueur j, int dir, int pas) {
        ...
        if (canPlayTour(posX, posY, dir, pas)) {
            if (dir == 1) j.positionX -= pas; // UPMOV
            else if (dir == 2) j.positionY += pas; // RIMOV
            else if (dir == 3) j.positionX += pas; // DOMOV
            else j.positionY -= pas; // LEMOV
        }
    }
